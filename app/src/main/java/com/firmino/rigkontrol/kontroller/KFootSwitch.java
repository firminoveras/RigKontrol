...
public class KFootSwitch extends LinearLayout {
    ...
    public KFootSwitch(@NonNull Context context) {
        super(context);
        setup("0", 0, 127, 0, false);
        init();
    }

    public KFootSwitch(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.KFootSwitch, 0, 0);
        setup(
                ta.getString(R.styleable.KFootSwitch_k_button_description),
                ta.getInt(R.styleable.KFootSwitch_k_button_control_num, 0),
                ta.getInt(R.styleable.KFootSwitch_k_button_on_value, 127),
                ta.getInt(R.styleable.KFootSwitch_k_button_off_value, 0),
                ta.getBoolean(R.styleable.KFootSwitch_k_button_is_toggled, false)
        );
        ta.recycle();
    }
    ...
    private void init() {
        inflate(this.getContext(), R.layout.layout_kontroller_kfootswitch, this);
    ...
    private void showConfigsDialog() {
        View mDialogContent = LayoutInflater.from(this.getContext()).inflate(R.layout.layout_kontroller_kfootswitch_config, null);
        ...
        KNumberPicker dialog_ComponentNumber = mDialogContent.findViewById(R.id.Config_CC);
        dialog_ComponentNumber.setValue(mControllerNumber);
        ...
        mDialogContent.findViewById(R.id.Config_OK).setOnClickListener(l -> {
            setup(
                    ...
                    dialog_ComponentNumber.getValue(),
    ...
    public interface OnKButtonStateChangeListener {
        void onKButtonStateChangeListener(KFootSwitch button, int valueOn, int valueOff, boolean isOn, int controllerNumber);
    }
}
